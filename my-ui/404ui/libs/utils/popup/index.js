...
const hookTransition = (transition) =>{
  ...
  const getVueInstance = (el) =>{
    ...
    if(!instance){
      const textNode = el.previousSibling;
      if(textNode.__vue__){
...
const getScrollBarWidth = ()=>{
  ...
  const widthInnerScroll = inner.offsetWidth;
  outer.parentNode.removeChild(outer);

  return widthNoScroll - widthInnerScroll;
...
export default {
  ...
  created(){
    if(this.transition) hookTransition(this.transition)
  },
  beforeMount(){
  ...
  methods:{
    ...
    doClose(){
      ...
      this.opened = false;
      if(!this.transition) this.doAfterClose()
    },
    doAfterClose(){
      PopupManager.closeModal(this._popupId);
      this._closing = false;
